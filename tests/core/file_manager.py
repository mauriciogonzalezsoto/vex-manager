...
def create_vex_library() -> str:
    ...
    folder_path = os.path.join(
        home_path, "vex-manager-test", config.WrangleNodes.ATTRIB_WRANGLE.value[1]
    )
...
def create_new_file() -> None:
    ...
    # Default name.
    new_vex_file = file_manager.create_new_vex_file(library_path=folder_path)

    # Custom name.
    new_vex_file_custom_name = file_manager.create_new_vex_file(
        library_path=folder_path, name="test"
    )
